...
def create_valid_index00(tmp_path: Path) -> Callable[[int, int], Path]:
    ...
    def _create(num_files: int = 1, num_segments: int = 1) -> Path:
        ...
        for i in range(num_segments):
            start_time = int(datetime(2023, 1, 1, 12, 0, i, tzinfo=timezone.utc).timestamp())
            end_time = int(datetime(2023, 1, 1, 12, 0, i + 1, tzinfo=timezone.utc).timestamp())
            start_offset = i * 1024
            end_offset = (i + 1) * 1024
...
def create_valid_index00p(tmp_path: Path) -> Callable[[int], Path]:
    ...
    def _create(num_segments: int = 1) -> Path:
        ...
        header = struct.pack(
            "<QIIII1176s76sI",
            1, 3, 1, 1, 0, b"\x00" * 1176, b"\x00" * 76, 0,
        )
        ...
        segment_records = b""
        for i in range(num_segments):
            time_val = int(datetime(2023, 1, 1, 12, 0, i, tzinfo=timezone.utc).timestamp())
            start_offset = i * 100
            end_offset = (i + 1) * 100
            
            segment_record = struct.pack(
                "<4s4xQQ16xII48x",
                b"\x01\x02\x03\x04", time_val, time_val, start_offset, end_offset,
            )
            segment_records += segment_record
